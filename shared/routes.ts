...
export async function registerRoutes(
  ...
  app.get("/api/test123", (_req, res) => { res.json({test:true}); });
    ...
    try {
      ...
      const cards = data.itemSummaries
        .filter((item: any) => {
          ...
          return !title.includes("lot") && !title.includes("mystery") && !title.includes("reprint") && !title.includes("custom") && !title.includes("bundle");
        })
        .map((item: any) => {
          ...
          const gradeMatch = title.match(/PSA\s*(\d+)/i) || title.match(/BGS\s*([\d.]+)/i) || title.match(/SGC\s*(\d+)/i);
          let grade = "Raw";
          if (gradeMatch) {
      ...
      // Deduplicate by simplified title
      const seen = new Set();
  ...
  app.post("/api/grading-roi", (req, res) => {
    try {
      ...
      if ([raw_price, psa9_price, psa10_price, grading_cost, psa10_probability].some((v: any) => v == null || isNaN(v))) {
      ...
      const break_even = profit_diff !== 0 ? Math.max(0, Math.min(100, (-psa9_profit / profit_diff) * 100)) : psa9_profit >= 0 ? 0 : 100;
      let verdict: string;
      ...
      res.json({ verdict, expected_profit: Math.round(expected_profit * 100) / 100, psa9_profit: Math.round(psa9_profit * 100) / 100, psa10_profit: Math.round(psa10_profit * 100) / 100, break_even_psa10_rate: Math.round(break_even * 10) / 10, explanation });
    } catch (err) {
