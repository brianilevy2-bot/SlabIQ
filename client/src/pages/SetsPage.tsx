  ...
  if (view === "list") {
    return (
      <div className="space-y-6 animate-fade-up">
        ...
        {showCreateSet && (
          <Modal title="New Set" subtitle="Define a set to track"
            onClose={() => { setShowCreateSet(false); resetCreateSet(); }}>
    ...
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => { setView("list"); setActiveSet(null); }}
      ...
      <div className="rounded-xl p-6" style={{ background: "var(--bg-card)", border: "1px solid var(--border-subtle)" }}>
        <div className="flex items-end justify-between mb-4">
          <div>
            <p className="text-[10px] uppercase tracking-widest mb-1" style={{ color: "var(--text-muted)" }}>— Completion</p>
        ...
        <div className="w-full h-2 rounded-full overflow-hidden mb-5" style={{ background: "rgba(255,255,255,0.05)" }}>
      ...
      {setCards.length === 0 ? (
        <div className="text-center py-16 rounded-xl"
          ...
          <button onClick={() => setShowAddCard(true)} className="btn-gold px-5 py-2 text-sm">+ Add Cards</button>
        </div>
      ) : (
      ...
      {showAddCard && (
        <Modal title="Add Cards" subtitle={activeSet!.name}
          ...
          {addMode === "search" && (
            <div className="space-y-3">
              ...
              {searching && <p className="mono text-[11px]" style={{ color: "var(--text-muted)" }}>Searching eBay...</p>}
              ...
              {searchQuery.length >= 2 && !searching && searchResults.length === 0 && (
                <p className="mono text-[11px]" style={{ color: "var(--text-muted)" }}>No results — try Manual or Paste List.</p>
              )}
            </div>
          )}
          ...
          {addMode === "manual" && (
            <form onSubmit={addCardToSet} className="space-y-4">
              ...
              <div className="flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer"
                ...
                <span className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>I already own this card</span>
              </div>
                  ...
                  <div className="rounded-lg overflow-hidden max-h-48 overflow-y-auto"
                    ...
                    {parsedCards.slice(0, 50).map((c, i) => (
                      <div key={i} className="flex items-center gap-3 px-3 py-2"
                        ...
                        {c.card_number && (
                          <span className="mono text-[11px] w-8 shrink-0" style={{ color: "var(--text-muted)" }}>#{c.card_number}</span>
                        )}
...
function CardRow({ card, onToggle, onDelete }: { card: SetCard; onToggle: () => void; onDelete: () => void }) {
  return (
    <div className="glass-card flex items-center gap-3 px-4 py-3 group">
      ...
      {card.est_cost != null && (
        <span className="mono text-xs shrink-0" style={{ color: card.owned ? "var(--text-muted)" : "var(--gold)" }}>
